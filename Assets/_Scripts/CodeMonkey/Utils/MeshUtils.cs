...
public static void AddToMeshArraysV2(Vector3[] vertices, Vector2[] uvs, int[] triangles, int index, Vector3 pos, float rot, Vector3 baseSize, Vector2 uv00, Vector2 uv11) {
		//Relocate vertices
		int vIndex = index*4;
		int vIndex0 = vIndex;
		int vIndex1 = vIndex+1;
        ...
        bool skewed = baseSize.x != baseSize.y;
        if (skewed) {
			vertices[vIndex0] = pos+GetQuaternionEulerV2(rot)*new Vector3(-baseSize.x,  baseSize.z);
			vertices[vIndex1] = pos+GetQuaternionEulerV2(rot)*new Vector3(-baseSize.x, -baseSize.z);
			vertices[vIndex2] = pos+GetQuaternionEulerV2(rot)*new Vector3( baseSize.x, -baseSize.z);
			vertices[vIndex3] = pos+GetQuaternionEulerV2(rot)*baseSize;
		} else {
			vertices[vIndex0] = pos+GetQuaternionEulerV2(rot-225)*baseSize;
			vertices[vIndex1] = pos+GetQuaternionEulerV2(rot-135)*baseSize;
			vertices[vIndex2] = pos+GetQuaternionEulerV2(rot- 45)*baseSize;
			vertices[vIndex3] = pos+GetQuaternionEulerV2(rot+ 45)*baseSize;
		}
	...
	private static void CacheQuaternionEulerV2() {
        if (cachedQuaternionEulerArr != null) return;
        cachedQuaternionEulerArr = new Quaternion[360];
        for (int i=0; i<360; i++) {
    ...
    private static Quaternion GetQuaternionEulerV2(float rotFloat) {
        int rot = Mathf.RoundToInt(rotFloat);
        rot = rot % 360;
        if (rot < 0) rot += 360;
        //if (rot >= 360) rot -= 360;
        if (cachedQuaternionEulerArr == null) CacheQuaternionEulerV2();
        return cachedQuaternionEulerArr[rot];
    }
